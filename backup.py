...
def recenter_hough(img_, frame_index, cell_index):
    w, h = img_.shape
    img = img_.copy()
    ...
    for i in circles[0, :]:
        ...
        if i[0] < w/4*3 and i[0] > w/4 and i[1] < h/4*3 and i[1] > h/4:
            cv2.circle(cimg, (i[0], i[1]), i[2], (255, 0, 0), 3)
            ''' Draw the centers of the circles '''
            cv2.circle(cimg, (i[0], i[1]), 2, (255, 0, 0), 5)
            cv2.imwrite('recenter/patch/patch{}_{}.jpg'.format(frame_index, cell_index), cimg)
            return i[0], i[1]
    # print(circles[0, :][0][0], circles[0, :][0][1])
    # return circles[0, :][0][0], circles[0, :][0][1]

def recenter_contour(img_, frame_index, cell_index):
    w, h = img_.shape
    img = img_.copy()
    img_c = img_.copy()
    img_ttt = img_.copy()
    ...
    _, img_t = cv2.threshold(img, 55, 255, cv2.THRESH_BINARY)
    # img_t = cv2.adaptiveThreshold(img,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY,11,2)
    cv2.imwrite('recenter_c/tmp/tmp{}_{}.jpg'.format(frame_index, cell_index), img_t)
...
def get_rotation_template(cell_index, frame_index, angle, cells_loc):
    ...
    row, col = img.shape
    cx, cy = recenter_contour(img, frame_index, cell_index)
    adjust_x = row/2 - cx
    adjust_y = col/2 - cy
...
def single_cell_direction(cell_index, frame_index, cells_loc):
    ...
    for degree in degrees:
        img_compare = get_rotation_template(cell_index, frame_index, degree, cells_loc)
    ...
    print(score_list)
    plot_mse(score_list, cell_index, frame_index)
